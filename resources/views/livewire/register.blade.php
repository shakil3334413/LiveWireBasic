<div class="wrapper">
    ...
    <form class="form-right" wire:submit.prevent="submit">
        <h2 class="text-uppercase">Registration form</h2>
        <div class="row">
            <div class="mb-3">
                <label>Name</label>
                <input type="text" wire:model="form.name" id="name" class="input-field">
                @error('form.name')<span class="text-danger">{{ $message }}</span> @enderror
            </div>
        </div>
        <div class="mb-3">
            <label>Your Email</label>
            <input type="email" class="input-field" wire:model="form.email">
            @error('form.email')<span class="text-danger">{{ $message }}</span> @enderror
        </div>
        <div class="row">
            <div class="col-sm-6 mb-3">
                <label>Password</label>
                <input type="password" wire:model="form.password" id="pwd" class="input-field">
                @error('form.password')<span class="text-danger">{{ $message }}</span> @enderror
            </div>
            <div class="col-sm-6 mb-3">
                <label>Confirm Password</label>
                <input type="password" wire:model="form.password_confirmation" id="cpwd" class="input-field">
                {{-- @error('form.password')<span class="text-danger">{{ $message }}</span> @enderror --}}
            </div>
        </div>
        <div class="mb-3">
